            ...
            {% block menu_categories %}
                ...
                <li>
                    ...
                    <ul>
                        ...
                        <li style="cursor:pointer;" onClick="javascript:changeChecked('category_{{temp.pk}}');"><input id="category_{{temp.pk}}" type="checkbox" name="categories" value="{{temp.pk}}" {% if temp.pk in selected_categories %}checked="checked"{% endif %}/> {{temp.title}}</li>
                        {% endfor %}
                    </ul>
                </li>
                {% endif %}
                ...
                <li>
                    ...
                    <ul>
                        {% for temp in menu_countries %}
                            <li style="cursor:pointer;" onClick="javascript:changeChecked('country_{{temp.pk}}');"><input id="country_{{temp.pk}}" type="checkbox" name="countries" value="{{temp.pk}}" {% if temp.pk in selected_countries %}checked="checked"{% endif %}/> {{temp.get_html|safe}}</li>
                        {% endfor %}
                    </ul>
                </li>
                {% endif %}
            ...
            <li>
                ...
                <ul class="no-list">
                    {% for temp in menu_producers %}
                        <li style="cursor:pointer;" onClick="javascript:changeChecked('producer_{{temp.pk}}');"><input id="producer_{{temp.pk}}" type="checkbox" name="producers" value="{{temp.pk}}" {% if temp.pk in selected_producers %}checked="checked"{% endif %}/><img alt="{{item.producer.country.short_desc}}" src="{{temp.producer.country.pic.url_13x20}}"/> {{temp.title}}</li>
                    {% endfor %}
                </ul>
            </li>
            {% endif %}
            ...
            <li>
                ...
                <ul>
                    {% for temp in menu_materials %}
                        <li style="cursor:pointer;" onClick="javascript:changeChecked('material_{{temp.pk}}');"><input id="material_{{temp.pk}}" type="checkbox" name="materials" value="{{temp.pk}}" {% if temp.pk in selected_materials %}checked="checked"{% endif %}/> {{temp.title}}</li>
                    {% endfor %}
                </ul>
            </li>
            {% endif %}
