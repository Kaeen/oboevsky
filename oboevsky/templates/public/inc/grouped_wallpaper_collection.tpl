                        ...
                        {% if forloop.counter == 9 %}
                            ...
                            function fetch_items_{{item.id}}() {
                                ...
                                $.getJSON('http://oboevsky.ru/get-items-collection/{{item.id}}{% for t in group.2|slice:"8:" %},{{t.id}}{% endfor %}/',
									...
									.fail(function() {
										alert('Произошла ошибка при попытке загрузки коллекции обоев. Пожалуйста, попробуйте обновить страницу и загрузкить их снова, а также проверьте соединение с интернетом. ');
										$('#trigger-{{item.id}}').text('Ошибка!');
									});
                            }
                            </script>
